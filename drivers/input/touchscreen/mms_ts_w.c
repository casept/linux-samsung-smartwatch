...
// TODO: Remove this, there's probably a framework for handling this cleanly in newer kernels (interactive governor?)
#define TOUCH_BOOSTER 0

// TODO: Make this compile, as it could be a useful debug feature
...
#include <mach/dev.h>
#endif
// TODO: This header has been removed, reimplement it's features in a mainline-friendly way
...
enum { SEC_NONE = -1, SEC_BOOT = 0, SEC_CORE, SEC_CONF, SEC_LIMIT };
...
#if ISC_DL_MODE
static int mms_read_config(struct i2c_client *client, u8 *buf, u8 *get_buf,
			   int len);
...
static void mms_reboot(struct mms_ts_info *info)
{
	...
	// TODO: i2c_unlock_adapter(adapter);
}
...
static void mms_config_set(void *context)
{
	...
	if ((conf_hdr->core_version & 0xff) != info->fw_ic_ver[1]) {
		mms_reboot(info);
		dev_err(&client->dev,
			"mfsp-version is not correct : 0x%x 0x%x :: 0x%x 0x%x\n",
	...
	for (i = 0;; i++, offset += MMS_MFSP_OFFSET) {
		conf_item[i] =
			(struct mms_config_item *)(info->config_fw + offset);
		...
		if (conf_item[i]->datasize == 1) {
			...
			mms_config_flash(info, config_flash_data, 2,
					 "config-data1");
			mms_read_config(client, config_flash_set, cmp_data,
					conf_item[i]->datasize);
			...
			mms_config_flash(info, config_flash_data, 3,
					 "config-data2");
			mms_read_config(client, config_flash_set, cmp_data,
					conf_item[i]->datasize);
			if (mms_verify_config(info, &config_flash_data[1],
			...
			mms_config_flash(info, config_flash_data, 5,
					 "config-data4");
			mms_read_config(client, config_flash_set, cmp_data,
					conf_item[i]->datasize);
...
static int mms_config_get(struct mms_ts_info *info, u8 mode)
{
	...
	if (mode == REQ_FW) {
		snprintf(fw_path, MAX_FW_PATH, "%s%s", TSP_FW_DIRECTORY,
			 TSP_FW_CONFIG_NAME);
...
static int mms_flash_section(struct mms_ts_info *info, struct mms_fw_img *img,
			     const u8 *data)
{
	struct i2c_client *client = info->client;
			...
			u8 update_mode)
{
	...
	dev_info(&client->dev,
		 "MMS-128S Before FW update : [0x%02x][0x%02x][0x%02x]",
	...
	for (i = 0; i < fw_hdr->section_num;
		...
		if (update_mode == FORCED_UPDATE) {
			...
			dev_info(
				&client->dev,
				"section [%d] forced update mode. ver : 0x%x, bin : 0x%x\n",
				img[i]->type, ver[img[i]->type], target[i]);
		} else if (ver[img[i]->type] == 0xff) {
			update_flag = true;
			dev_info(
				&client->dev,
				"tsp ic fw error : section [%d] is need to be updated\n",
				img[i]->type);
			...
			dev_info(
				&client->dev,
				"empty tsp ic : section [%d] is need to be updated\n",
				img[i]->type);
			...
			dev_info(&client->dev,
				 "section [%d] is up to date, ver : 0x%x\n", i,
				 target[i]);
		}
		...
		if (update_flag) {
			...
			ret = mms_flash_section(
				info, img[i], fw_data + fw_hdr->binary_offset);
			update_flag = false;
	...
	if (isc_enter_flag) {
		...
		if (get_fw_version_ic(client, ver)) {
			dev_err(&client->dev,
				"failed to obtain version after flash\n");
			ret = -1;
			goto out;
		} else {
...
static void work_mms_config_set(struct work_struct *work)
{
	struct mms_ts_info *info =
		container_of(work, struct mms_ts_info, work_config_set.work);
	mms_config_set(info);

#if TSP_TA_CALLBACK
	dev_info(&info->client->dev, "%s & TA %sconnect & noise mode %s!\n",
		 __func__, info->ta_status ? "" : "dis",
...
{
	struct mms_ts_info *info =
		container_of(work, struct mms_ts_info, work_dvfs_chg.work);
	int ret;
...
{
	struct mms_ts_info *info =
		container_of(work, struct mms_ts_info, work_dvfs_off.work);
	int ret;
...
static void reset_mms_ts(struct mms_ts_info *info)
{
	...
	dev_info(&client->dev, "%s & tsp %s & TA %sconnect & noise mode %s!\n",
		 ...
		 info->ta_status ? "" : "dis", info->noise_mode ? "on" : "off");
#else
...
static irqreturn_t mms_ts_interrupt(int irq, void *dev_id)
{
	...
	struct i2c_client *client = info->client;
	u8 buf[MAX_FINGERS * EVENT_SZ_8_BYTES] = {
		0,
	};
	int ret, i, sz;
	...
	if (buf[0] == MMS_NOTIFY_EVENT) { /* MMS_NOTIFY_EVENT */
		...
		if (!info->ta_status)
			dev_err(&client->dev,
				"TSP noise mode enter but ta off!\n");
...
static int mms_ts_fw_load(struct mms_ts_info *info)
{
	...
	if (!retries) {
		dev_err(&info->client->dev,
			"failed to flash firmware after retires\n");
	} else {
...
static void get_intensity_data(struct mms_ts_info *info)
{
	...
	for (i = 0; i < rx_num; i++) {
		...
		ret = i2c_smbus_write_i2c_block_data(info->client, w_buf[0], 3,
						     &w_buf[1]);
		if (ret < 0)
			goto err_i2c;
		usleep_range(1, 5);

		ret = i2c_smbus_read_i2c_block_data(info->client, CMD_RESULT_SZ,
						    1, &r_buf);
		if (ret < 0)
			goto err_i2c;
...
static void get_raw_data(struct mms_ts_info *info, u8 cmd)
{
	...
	while (retry) {
		...
		ret = i2c_smbus_read_i2c_block_data(info->client, 0x0F, 1,
						    &r_buf);
		if (ret < 0)
			goto err_i2c;

		ret = i2c_smbus_read_i2c_block_data(info->client, 0x10, 1,
						    &r_buf);
		if (ret < 0)
			goto err_i2c;
	...
	ret = i2c_smbus_read_i2c_block_data(info->client, CMD_RESULT_SZ, 1,
					    &r_buf);
	if (ret < 0)
		goto err_i2c;
	...
	if (cmd == MMS_VSC_CMD_CM_DELTA)
		...
		w_buf[1] = CMD_CM_ABS;
	ret = i2c_smbus_write_i2c_block_data(info->client, w_buf[0], 1,
					     &w_buf[1]);
	if (ret < 0)
		goto err_i2c;
	...
	if (ret < 0)
		goto err_i2c;
	ret = i2c_smbus_read_i2c_block_data(info->client, CMD_RESULT, 1,
					    &r_buf);
	if (ret < 0)
		goto err_i2c;
...
static void get_fw_ver_ic(void *device_data)
{
	...
	snprintf(buff, sizeof(buff), "ME00%02x%02x", info->fw_ic_ver[SEC_CORE],
...
static int mms_ts_probe(struct i2c_client *client)
{
	...
	if (client->dev.of_node) {
		info->pdata = devm_kzalloc(
			&client->dev, sizeof(struct melfas_tsi_platform_data),
			GFP_KERNEL);
		if (!info->pdata) {
			dev_err(&client->dev, "Failed to allocate memory\n");
			return -ENOMEM;
	...
	input_set_abs_params(input_dev, ABS_MT_TOUCH_MAJOR, 0, MAX_PRESSURE, 0,
			     0);
...
*/
	...
	// wake_lock_init(&info->tsp_wake_lock, WAKE_LOCK_SUSPEND,
	//	       "report_touch_wake_lock");
	...
	if (unlikely(info->reference == NULL || info->cm_abs == NULL ||
		     info->cm_delta == NULL || info->intensity == NULL)) {
		ret = -ENOMEM;
	...
	fac_dev_ts = device_create(sec_class, NULL, 0, info, "tsp");
...
static void mms_ts_remove(struct i2c_client *client)
{
...
static int mms_ts_resume(struct device *dev)
{
	...
	schedule_delayed_work(&info->work_config_set, msecs_to_jiffies(40));
#endif
...
static const struct i2c_device_id mms_ts_id[] = { { MELFAS_TS_NAME, 0 }, {} };
