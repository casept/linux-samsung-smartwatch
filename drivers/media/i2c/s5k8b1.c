...
static struct v4l2_mbus_framefmt *__s5k8b1_get_format(
		struct s5k8b1 *sensor, struct v4l2_subdev_state *sd_state,
				  ...
				  struct v4l2_subdev_format *fmt)
{
	...
	mf = __s5k8b1_get_format(sensor, sd_state, fmt->pad, fmt->which);
	if (mf) {
...
static const struct v4l2_subdev_pad_ops s5k8b1_pad_ops = {
...
static const struct v4l2_subdev_core_ops s5k8b1_core_ops = {
	.s_power = s5k8b1_s_power,
};

static const struct v4l2_subdev_ops s5k8b1_subdev_ops = {
...
static int s5k8b1_probe(struct i2c_client *client)
{
	...
	gpio = of_get_named_gpio(dev->of_node, "xshutdown-gpios", 0);
	if (!gpio_is_valid(gpio)) {
		dev_err(dev, "failed to probe: failed to get valid GPIO, gpio = %d\n", gpio);
		return gpio;
	}
	...
	ret = devm_gpio_request_one(dev, gpio, GPIOF_OUT_INIT_LOW,
						S5K8B1_DRV_NAME);
	if (ret < 0) {
		dev_err(dev, "failed to probe: failed to request GPIO, ret = %d\n", ret);
		return ret;
	}
	...
	ret = devm_regulator_bulk_get(&client->dev, S5K8B1_NUM_SUPPLIES,
				      sensor->supplies);
	if (ret < 0) {
		dev_err(dev, "failed to probe: failed to get regulator, ret = %d\n", ret);
		return ret;
	}

	sensor->clock = devm_clk_get(dev, S5K8B1_CLK_NAME);
	if (IS_ERR(sensor->clock)) {
		dev_err(dev, "defering probe: failed to request clock\n");
		return -EPROBE_DEFER;
	}
...
static void s5k8b1_remove(struct i2c_client *client)
{
